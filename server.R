...
server <- function(input, output, session) {
  ...
  callModule(module = InfoBox,id="info",reactive(data$data))
  callModule(module = RaceChart,id="top",reactive(data$data))
  observeEvent(input$sidebarmenu, {
    if(input$sidebarmenu == "Visualization"){
      callModule(module = AreaChart,id="area",reactive(data_subset$data))  
      callModule(module = SpChart,id="sp",reactive(data_subset$data)) 
      callModule(module = FlagChart,id="flag",reactive(data_subset$data))
      callModule(module = PieMarChart,id="marine",reactive(data_subset$data))
      callModule(module = PieInChart,id="inland",reactive(data_subset$data))
    }})
